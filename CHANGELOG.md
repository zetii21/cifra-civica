...
- Iteración UX 4: continuidad del escenario del laboratorio — «Guardar en este dispositivo» explícito (IndexedDB local, coherente con la garantía opt-in de /privacy) con banner de reanudación al volver (retomar o borrar), «Copiar enlace» que transporta las palancas en el fragmento de la URL (el navegador nunca lo envía al servidor) con códec validado — versión de modelo comprobada, claves desconocidas descartadas y valores recortados al rango de cada palanca (`tests/lab-scenario.test.ts`) —, y columna de territorio fija durante el desplazamiento horizontal de la tabla en pantallas estrechas.
